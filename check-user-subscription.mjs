...
async function checkUserSubscription() {
  try {
    console.log(`🔍 Checking subscription status for: ${targetEmail}`)
    ...
    if (userSubscriptions.length === 0) {
      ...
      console.log('This explains why the account page shows "Trial Expired" status')
      return
    }
    ...
    for (const sub of userSubscriptions) {
      ...
      console.log(`  Updated: ${sub.updatedAt?.toLocaleDateString()}`)
